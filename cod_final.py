...
def ipc_provincias() -> dict[str, float]:
    INE_IPC_PROV_URL = "https://servicios.ine.es/wstempus/js/es/DATOS_TABLA/24081?tip=AM"
    r = requests.get(INE_IPC_PROV_URL, headers={"Accept": "application/json"}, timeout=30)
    r.raise_for_status()
    data = r.json()
    ...
    for serie in data:
        ...
        puntos = serie.get("Data") or []
        if not nombre or not puntos:
            continue

        # En muchas tablas del INE, el nombre empieza por la provincia (antes del primer ".")
        provincia = nombre.split(".")[0].strip()

        # último dato disponible
        valor = puntos[-1].get("Valor")
        try:
            v = float(str(valor).replace(",", "."))
        except Exception:
            continue

        # filtra España si aparece
        if provincia.lower() in {"españa", "total nacional"}:
            continue

        out[provincia] = v

    if not out:
        raise RuntimeError("No se extrajo nada. Puede haber cambiado el formato de la tabla.")
    return out
